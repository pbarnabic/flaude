 ...
 */
export const ArtifactParsingUtilsV2 = {

    /**
     * Simplified smart content concatenation for artifact continuations
     */
    smartConcat: (existing, incoming) => {
        ...
        // Get the last line of existing content and first line of incoming
        const existingLines = existing.split('\n');
        const incomingLines = incoming.split('\n');
        const lastLine = existingLines[existingLines.length - 1];
        const firstLine = incomingLines[0];

        // Check if the first line of incoming starts with the last line of existing
        if (firstLine.startsWith(lastLine) && lastLine.length > 0) {
            // Replace the last line with the first line (it's a continuation/overlap)
            existingLines[existingLines.length - 1] = firstLine;
            incomingLines.shift(); // Remove the first line from incoming

            // Rejoin everything
            const result = existingLines.join('\n');
        ...
        // No overlap detected - check if we need a newline
        if (existing.endsWith('\n') || existing.length === 0) {
            // Existing ends with newline or is empty - just concatenate
            return existing + incoming;
        } else {
            // Existing doesn't end with newline - assume continuation on same line
            return existing + incoming;
        }
    },
                ...
                if (idMatch) {
                    ...
                    if (isContinuation && existingVersions[idMatch[1]]) {
                        ...
                        if (lastVersion && lastVersion.content) {
                            ...
                            artifactContent = ArtifactParsingUtilsV2.smartConcat(
                                lastVersion.content,
                                artifactContent
                            );
                        }
